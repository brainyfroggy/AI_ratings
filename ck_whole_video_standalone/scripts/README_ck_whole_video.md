...
OpenCV cannot decode a frame at the exact final timestamp of a clip. The
extractor handles this by stepping back one frame interval to grab the last
readable frame (the expression apex), so the full requested frame count is sent
— including the final frame. A `Warning: Cannot read frame at <N>ms` only prints
if every fallback also fails; the rating still succeeds.
