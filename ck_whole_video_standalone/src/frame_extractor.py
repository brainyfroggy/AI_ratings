...
def _read_frame_by_index(video_path: Path, frame_idx: int) -> Tuple[bool, Optional[np.ndarray], float]:
    """Read a frame by index from a fresh capture and return its timestamp."""
    ...
    try:
        ...
        ret, frame = cap.read()
        return ret, frame, cap.get(cv2.CAP_PROP_POS_MSEC)
    finally:
        cap.release()
...
class FrameExtractor:
        ...
        try:
            ...
            if not ret:
                ...
                if fps and fps > 0 and frame_count > 0:
                    frame_idx = min(
                        frame_count - 1,
                        max(0, int(round(timestamp_ms * fps / 1000.0))),
                    )
                    ret, frame, _ = _read_frame_by_index(video_path, frame_idx)
            if not ret:
                raise ValueError(
    ...
    def extract_frames_with_context(
        ...
        try:
            ...
            fps = cap.get(cv2.CAP_PROP_FPS)
            frame_count = int(cap.get(cv2.CAP_PROP_FRAME_COUNT) or 0)
            frame_step_ms = 1000.0 / fps if fps and fps > 0 else 40.0
            for ts in timestamps:
                cap.set(cv2.CAP_PROP_POS_MSEC, ts)
                ret, frame = cap.read()
                # OpenCV usually cannot decode a frame at the exact final
                # timestamp (ts == duration): the seek lands at/after the last
                # frame's presentation time and read() fails. Step back by whole
                # frame intervals to grab the last decodable frame (the apex of a
                # CK expression sequence) instead of dropping it.
                read_ts = ts
                attempts = 0
                while not ret and read_ts > 0 and attempts < 3:
                    read_ts = max(0, read_ts - frame_step_ms)
                    cap.set(cv2.CAP_PROP_POS_MSEC, read_ts)
                    ret, frame = cap.read()
                    attempts += 1
                if not ret and fps and fps > 0 and frame_count > 0:
                    frame_idx = min(
                        frame_count - 1,
                        max(0, int(round(ts * fps / 1000.0))),
                    )
                    ret, frame, pos_ms = _read_frame_by_index(video_path, frame_idx)
                    if ret:
                        read_ts = pos_ms if pos_ms > 0 else frame_idx * frame_step_ms
                if ret:
                    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                    frames.append((int(round(read_ts)), Image.fromarray(frame_rgb)))
                else:
